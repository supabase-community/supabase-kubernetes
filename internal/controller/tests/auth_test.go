...
var _ = Describe("Auth API", func() {
	It("requires a local immutable Project reference and permits zero replicas", func() {
		...
		c := &core.Auth{ObjectMeta: metav1.ObjectMeta{Name: "arbitrary-name", Namespace: ns}}
		...
		defaults := &core.Auth{ObjectMeta: metav1.ObjectMeta{Name: "defaults", Namespace: ns}, Spec: c.Spec}

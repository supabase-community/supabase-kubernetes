...
func (r *Reconciler) reconcileDeployment(ctx context.Context, state *defaults.Context, db *core.ResolvedDatabase) error {
	deployment, err := authdefaults.AuthDeployment(state, db)
	if err != nil {
	...
	return component.Ensure(ctx, r.Client, deployment, state.Owner, reconciler.MutateDeployment(), "Auth Deployment")
}
